 ...
 *
 * Creates a Stripe Checkout Session in EMBEDDED mode for the MoLivite
 * 1-Month Supply and returns its client secret. The frontend mounts the
 * form with `stripe.initEmbeddedCheckout({ clientSecret })`.
 *
 ...
 *
 * Price and product are fixed server-side. The client can never choose what
 * it pays — only how many.
 */
...
export const handler = async (event) => {
  ...
  const params = (withSuffix) => ({
    ...
    payment_intent_data: {
      description: `MoLivite — 1-Month Supply (60 capsules) × ${quantity}`,
      metadata: { product_id: PRODUCT.id, quantity: String(quantity) },
